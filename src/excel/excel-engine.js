...
HSE.excel = {
  ...
  record(row, map, module, sheet, index) {
    ...
    if (module === 'safety') return { ...base, incidentType: get('incidentType') || base.category, findingType: get('findingType'), severity: base.risk, description: base.finding, manHours: h.num(row.ManHours || row['Man Hours']), lostDays: h.num(row.LostDays || row['Lost Days']), shift: row.Shift || row.shift || '', employee: row.Employee || '', vehicleNumber: row['Vehicle Number'] || '', licensePlate: row['License Plate'] || '', remark: get('remark') };
    ...
    if (module === 'permit') {
      ...
      const normalizePermit = (value) => /ketinggian|working at height/i.test(value) ? 'Ketinggian' : /tangki|tank/i.test(value) ? 'Tangki' : /hot work|kerja panas/i.test(value) ? 'Kerja Panas' : /electrical|listrik/i.test(value) ? 'Kerja Listrik' : /alat berat|heavy equipment|lifting/i.test(value) ? 'Alat Berat' : /galian|excavation/i.test(value) ? 'Galian' : /confined space|ruang terbatas/i.test(value) ? 'Ruang Terbatas' : h.clean(value);
      const permitTypes = rawPermit.split(/[,;/]+/).map(normalizePermit).filter(Boolean);
      return { ...base, permitType: permitTypes.join(', '), permitTypes, permitNumber: get('permitNumber'), contractor: row.Contractor || row.Vendor || '', vendor: row.Vendor || row.Contractor || '', issueDate: h.date(row['Issue Date']) || base.date, closeDate: h.date(row['Close Date']), expireDate: h.date(row['Expire Date']) || h.date(row['Close Date']), approver: row.Approver || '', remark: get('remark') };
    }
