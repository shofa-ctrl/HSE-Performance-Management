...
HSE.pages = {
  ...
  drawExecutive() {
    ...
    const fiveRByDept = h.groupAvg(d.fiveR || [], 'department', 20, (r) => Number(r.averageScore) || 0);
    const fiveRDepartments = fiveRByDept.map(([department]) => department);
    const fiveRDepartmentMonthly = fiveRDepartments.map((department, index) => ({
      label: department,
      data: HSE.config.months.map((_, month) => Number(h.avg((d.fiveR || []).filter((r) => r.department === department && r.date?.getMonth() === month).map((r) => Number(r.averageScore) || 0)).toFixed(2))),
      backgroundColor: ['#2563eb', '#16a34a', '#d97706', '#dc2626', '#7c3aed', '#0891b2', '#64748b', '#db2777'][index % 8],
      borderRadius: 4,
    }));
    const fiveRMonthly = HSE.config.months.map((_, i) => Number(h.avg((d.fiveR || []).filter((r) => r.date?.getMonth() === i).map((r) => Number(r.averageScore) || 0)).toFixed(2)));
    const series = [h.byMonth(findings), h.byMonth(d.safety), HSE.config.months.map((_, i) => h.pct(d.capa.filter((r) => r.date?.getMonth() === i && r.status === 'Closed').length, d.capa.filter((r) => r.date?.getMonth() === i).length)), h.byMonth(findings), HSE.config.months.map(() => k.safetyScore), HSE.config.months.map(() => k.complianceScore), fiveRMonthly, HSE.config.months.map(() => k.environmentalScore), h.byMonth(d.project, (r) => r.progress / 100), h.byMonth(d.permit), HSE.helpers.group(findings, 'department').map(([, v]) => v), ['Critical', 'High', 'Medium', 'Low'].map((r) => findings.filter((x) => x.risk === r).length), HSE.helpers.group(findings, 'category').map(([, v]) => v), HSE.helpers.group(d.safety, 'incidentType').map(([, v]) => v), fiveRByDept.map(([, v]) => Number(v.toFixed(2))), HSE.helpers.group(d.capa, 'status').map(([, v]) => v), HSE.helpers.group(findings, 'department').map(([, v]) => v)];
    const labels = [HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.config.months, HSE.helpers.group(findings, 'department').map(([x]) => x), ['Critical', 'High', 'Medium', 'Low'], HSE.helpers.group(findings, 'category').map(([x]) => x), HSE.helpers.group(d.safety, 'incidentType').map(([x]) => x), fiveRByDept.map(([x]) => x), HSE.helpers.group(d.capa, 'status').map(([x]) => x), HSE.helpers.group(findings, 'department').map(([x]) => x)];
    series.forEach((s, i) => {
      if (i === 14) HSE.charts.make(`execChart${i}`, 'bar', HSE.config.months, fiveRDepartmentMonthly, { scales: { x: { grid: { display: false } }, y: { beginAtZero: true, max: 5, ticks: { stepSize: 1 } } } });
      else if ([11, 12, 13, 15].includes(i)) HSE.charts.doughnut(`execChart${i}`, labels[i], s);
      else if ([10, 14, 16].includes(i)) HSE.charts.bar(`execChart${i}`, labels[i], s, i === 14 ? 'Average 5R Score' : 'Total', i === 14 ? '#16a34a' : '#2563eb', true);
  ...
  drawModule(page) {
    ...
    if (page === 'analytics') {
      ...
      const fiveRByDept = h.groupAvg(fiveR, 'department', 20, (r) => Number(r.averageScore) || 0);
      const fiveRByArea = h.groupAvg(fiveR, 'area', 20, (r) => Number(r.averageScore) || 0);
  ...
  drawFiveRAndApd() {
    ...
    const apd = findings.filter((r) => /inspeksi apd|apd/i.test(`${r.auditType} ${r.findingType} ${r.category}`));
    if (fiveR.length) {
      const byDept = h.groupAvg(fiveR, 'department', 20, (r) => Number(r.averageScore) || 0);
      const byArea = h.groupAvg(fiveR, 'area', 20, (r) => Number(r.averageScore) || 0);
